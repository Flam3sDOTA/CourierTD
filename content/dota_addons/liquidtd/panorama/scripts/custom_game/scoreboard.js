...
var g_bAnimatingScoreboardFinished = false;
...
function OnRoundStart() {
	...
	g_bAnimatingScoreboardFinished = false;
...
function AnimateEventPoints(scoreData) {
	...
	var nTargetValue = scoreData[Game.GetLocalPlayerID().toString() + "PointTotal"]
	var nCurrentValue = $("#TowerAndBagContainer").FindChildInLayoutFile("PointsTotalValue").GetAttributeInt("value", 0);
	var nStep = Math.max(5, Math.floor((nTargetValue - nCurrentValue) / 10))

	if (nCurrentValue != nTargetValue) {
		var nNewValue = Math.min(nCurrentValue + nStep, nTargetValue);
...
function FinalWait() {
	if ((g_flNextAnimTime + 2.0) > Game.Time())
		return;

	g_bAnimatingScoreboardFinished = true;
}
...
function UpdateScoreboard(scoreData) {
	if (typeof scoreData != 'undefined') {
		...
		for (i = 0; i < 5; i++) {
			...
			var playerRow = $("#HoldoutScoreboard").FindChildInLayoutFile(playerRowPanelPrefix);
			var name = scoreData[i].name;
			var kills = scoreData[i].kills;
			var deaths = scoreData[i].deaths;
			var castles = scoreData[i].castles;
			var units = scoreData[i].units.current + "/" + scoreData[i].units.max;
			/*
			$.Msg("Name: " + name);
			$.Msg("Kills: " + kills);
			$.Msg("Deaths: " + deaths);
			...
			*/
			...
			if (playerRow !== null) {
				...
				var playerCastles = playerRow.FindChildInLayoutFile(playerRowPanelPrefix + "Saves");
				playerCastles.text = castles;

				var playerUnits = playerRow.FindChildInLayoutFile(playerRowPanelPrefix + "Bags");
				playerUnits.text = units;
			}
		}
	}
}

function UpdateScoreboard_Nettables(scoreData) {
	if ($.GetContextPanel().BHasClass("round_over") || $.GetContextPanel().BHasClass("flyout_scoreboard_visible")) {
		$.Schedule(Math.min(1, g_flNextAnimTime - Game.Time()), UpdateScoreboard);
	}
	...
	if (typeof scoreData != 'undefined') {
		...
		//$.Msg( "scoredata "+JSON.stringify(scoreData) );
		...
		if (scoreData["RoundOver"] === 1) {
			if (g_bRoundEndInit === false) {
				...
				$("#TowerAndBagContainer").FindChildInLayoutFile("PointsThisRoundValue").text = scoreData[Game.GetLocalPlayerID().toString() + "PointReward"];
				$("#TowerAndBagContainer").FindChildInLayoutFile("PointsTotalValue").text = scoreData[Game.GetLocalPlayerID().toString() + "PointTotal"] - scoreData[Game.GetLocalPlayerID().toString() + "PointReward"];
			...
			if (g_bAnimatingScoreboardFinished === false) {
				//$.Msg( "Animating Final Wait" );
				FinalWait();
				return;
			}
			//$.Msg( "Done Animating Scoreboard" );
...
function CloseHoldoutScoreboard() {
	$.DispatchEvent("DOTAHUDToggleScoreboard")
...
(function () {
	...
	GameEvents.Subscribe("update_scoreboard", UpdateScoreboard);
})();
