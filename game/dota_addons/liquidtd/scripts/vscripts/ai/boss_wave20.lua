function Spawn(keys)
  ...
  Timers:CreateTimer(.1, function()
	...
	GhostShroud = thisEntity:FindAbilityByName("necrolyte_sadist")
...
function thisEntity:AIThink()
  ...
  if GhostShroud ~= nil and GhostShroud:IsFullyCastable() and self:GetHealthPercent() <= 80 then
	CastGhostShroud()
  elseif GhostShroud ~= nil and GhostShroud:IsFullyCastable() and self:GetHealthPercent() <= 30 then
    CastGhostShroud()
  end
...
function CastGhostShroud()
		ExecuteOrderFromTable({
			UnitIndex = thisEntity:entindex(),
			OrderType = DOTA_UNIT_ORDER_CAST_NO_TARGET,
			AbilityIndex = GhostShroud:entindex()
		})
end