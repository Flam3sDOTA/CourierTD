function Spawn(keys)
  ...
  Timers:CreateTimer(.1, function()
	...
	ResonantPulse = thisEntity:FindAbilityByName("void_spirit_resonant_pulse")
...
function thisEntity:AIThink()
  ...
  if ResonantPulse ~= nil and ResonantPulse:IsFullyCastable() and self:GetHealthPercent() <= 50 then
	CastResonantPulse()
  elseif ResonantPulse ~= nil and ResonantPulse:IsFullyCastable() and self:GetHealthPercent() <= 10 then
    CastResonantPulse()
  end
...
function CastResonantPulse()
		ExecuteOrderFromTable({
			UnitIndex = thisEntity:entindex(),
			OrderType = DOTA_UNIT_ORDER_CAST_NO_TARGET,
			AbilityIndex = ResonantPulse:entindex()
		})
end