...
function modifier_lifestealer_rage_lua:OnCreated( kv )
	-- references
	self.as_bonus = self:GetAbility():GetSpecialValueFor( "movement_speed_bonus" ) -- special value
	if IsServer() then
		self:PlayEffects()
	end
end
...
function modifier_lifestealer_rage_lua:	GetModifierMoveSpeedBonus_Constant()
